...
const BattleRoyale = ({ tournamentId }) => {
  ...
  // Edit Team Modal
 // EditTeamModal Component with fixed inputs
const EditTeamModal = () => {
 ...
 // Add Match Results Form
 const AddMatchResultsForm = () => {
  if (!showAddMatchForm) return null;

  return (
    <div className="fixed inset-0 bg-black/80 z-[60] flex items-center justify-center backdrop-blur-sm transition-all duration-300 overflow-y-auto">
      <div className="bg-secondary shadow-2xl w-full max-w-4xl mx-4 overflow-hidden relative my-8">
        <div className="px-6 pt-4 pb-3 border-b border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-xs text-gray-400 uppercase tracking-wider">New Match Results</span>
              <h2 className="text-lg font-bold text-white uppercase tracking-wide leading-tight">
                {tournamentInfo ? tournamentInfo.name : 'Tournament'} - Match Results
              </h2>
            </div>
        ...
        <div className="px-6 py-4 max-h-[60vh] overflow-y-auto">
          <div className="bg-gray-900/50 p-4 mb-4 rounded-lg">
            <p className="text-sm text-gray-300">
              Enter the placement (position) and kill count for each team in this match.
              {settings && (
                <>
                  <br />
                  <span className="text-primary">
                    Points system: {settings.kill_points} point(s) per kill, placement points based on final position.
                  </span>
                </>
              )}
            </p>
          ...
          <table className="w-full text-white">
            ...
            <tbody className="divide-y divide-gray-800">
              {newMatchResults.map((result) => (
                <tr key={result.team_id} className="hover:bg-gray-800/30">
                  <td className="py-3 px-4">{result.team_name}</td>
                  <td className="py-3 px-4">
                    <input
                      type="number"
                      min="1"
                      max={newMatchResults.length}
                      value={result.position || ''}
                      onChange={(e) => handleMatchResultChange(result.team_id, 'position', e.target.value)}
                      className="w-full bg-gray-800 text-white text-center py-1 px-2 rounded-md focus:outline-none focus:ring-1 focus:ring-primary"
                    />
                  </td>
                  <td className="py-3 px-4">
                    <input
                      ...
                      min="0"
                      value={result.kills || ''}
                      onChange={(e) => handleMatchResultChange(result.team_id, 'kills', e.target.value)}
                      className="w-full bg-gray-800 text-white text-center py-1 px-2 rounded-md focus:outline-none focus:ring-1 focus:ring-primary"
                    />
                  </td>
                </tr>
        ...
        <div className="bg-gray-900/80 px-6 py-4 flex justify-between border-t border-gray-800/50">
          ...
          <button
            ...
            className="bg-primary hover:bg-primary/90 text-white py-2 px-4 angular-cut transition duration-200 flex items-center"
          >
            {savingMatch ? (
              <>
      ...
      <div className="fixed inset-0 bg-secondary z-50 overflow-y-auto">
        <div className="container mx-auto p-4 md:p-8">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-3xl md:text-4xl font-custom tracking-wider text-primary"></h2>
          ...
          {/* <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
            ...
            <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
              <button
                onClick={initializeNewMatch}
                className="bg-primary hover:bg-primary/90 text-white py-2 px-4 angular-cut transition duration-200 flex items-center justify-center"
              >
              ...
              <div className="relative w-full md:w-64">
                ...
                <input
                  ...
                  className="w-full bg-dark text-white py-2 px-8 angular-cut focus:outline-none focus:ring-2 focus:ring-primary"
                />
              </div>
            </div>
          </div> */}
          ...
          <div className="bg-dark angular-cut p-4 md:p-6 shadow-lg mb-6">
            ...
            {/* {settings && (
              <div className="mb-6 p-4 rounded-lg bg-gray-900/50">
                <h4 className="text-lg font-semibold mb-2">Tournament Settings</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-gray-800/30 p-3 rounded-lg">
                    <p className="text-gray-400 text-xs uppercase mb-1">Points per Kill</p>
                    <p className="text-xl text-primary font-bold">{settings.kill_points}</p>
                  </div>
                  <div className="bg-gray-800/30 p-3 rounded-lg">
                    <p className="text-gray-400 text-xs uppercase mb-1">Total Matches</p>
                    <p className="text-xl text-primary font-bold">{settings.match_count || 0}</p>
                  </div>
                  <div className="bg-gray-800/30 p-3 rounded-lg">
                    <p className="text-gray-400 text-xs uppercase mb-1">1st Place Points</p>
                    <p className="text-xl text-primary font-bold">
                      {settings.placement_points_distribution?.["1"] || "N/A"}
                    </p>
                  </div>
                </div>
              </div>
                      ...
                      >
                        ...
                        <td className="py-4 text-center">
                          <div className="transition-all duration-300 text-primary font-bold text-lg hover:font-bold ">
                            {team.total_points} <span className="ml-2 text-xs font-normal">pts</span>
                          </div>
                        </td>
