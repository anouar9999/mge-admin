...
const BattleRoyale = () => {
  ...
  const [newTeam, setNewTeam] = useState({ name: '', kills: '', classPoints: '', image: '' });
  ...
  const handleEditClick = (team) => {
    ...
    setEditingTeam({
      ...team,
      kills: team.kills.toString(),
      classPoints: team.classPoints.toString(),
    });
  ...
  const handleEditChange = (e) => {
    ...
    if (name === 'kills' || name === 'classPoints') {
      // If value is empty or numeric, update state
      ...
      // Otherwise ignore the input (non-numeric)
    } else {
  ...
  const handleEditSave = () => {
    // Calculate total and convert values to numbers
    const updatedTeam = {
      ...editingTeam,
      kills: parseInt(editingTeam.kills || 0),
      classPoints: parseInt(editingTeam.classPoints || 0),
  ...
  const EditTeamModal = () => {
    ...
    const currentTotal = parseInt(editingTeam.kills || 0) + parseInt(editingTeam.classPoints || 0);
    ...
    return (
      <div className="fixed inset-0 bg-black/75 z-[60] flex items-center justify-center backdrop-blur-sm transition-all duration-300">
        <div 
          className="bg-gradient-to-b from-secondary to-secondary/70 shadow-2xl w-full max-w-2xl mx-4 overflow-hidden relative animate-fadeIn"
        >
          {/* Top accent bar */}
          ...
          <div className="px-6 py-5">
            ...
            <div className="mt-8 text-center bg-gradient-to-b from-gray-800/70 to-secondary/70 py-6 rounded-lg shadow-inner relative overflow-hidden group hover:border-blue-500/30 transition-all duration-300 mx-4">
              ...
              <span className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-primary/75 to-primary text-5xl block my-2">{currentTotal}</span>
              ...
              <td className="py-4 w-1/6 text-center relative z-10 align-middle">
                <div
                  className={`transition-all duration-300 ${
                    index < 3 ? 'text-white font-free-fire' : 'text-primary font-free-fire'
