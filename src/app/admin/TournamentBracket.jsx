...
import { FaTrophy, FaChartBar, FaUserFriends, FaInfoCircle, FaMedal, FaCalendarAlt, FaFilter } from 'react-icons/fa';
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="text-xs text-gray-400 uppercase">
                    ...
                    <th className="py-2 w-12 text-center">W</th>
                    <th className="py-2 w-12 text-center">D</th>
                    <th className="py-2 w-12 text-center">L</th>
        ...
        <div className="flex overflow-x-auto pl-4 space-x-2">
          {Array.from({ length: maxRounds }, (_, index) => (
            <button
              ...
              className={`px-6 py-3 font-bold tracking-wider flex items-center relative overflow-hidden group ${
                currentRound === index
                  ? 'text-primary'
                  : 'text-white hover:text-primary'
              }`}
            ...
            >
              ...
              <div 
                className={`absolute inset-0 bg-black z-0 transition-all duration-300 ease-in-out ${
                  currentRound === index ? 'border border-primary' : 'border border-transparent'
                }`}
                style={{
                  clipPath: "polygon(0 0, 95% 0, 100% 100%, 5% 100%)"
                }}
        ...
        <table className="w-full border-collapse table-fixed">
          ...
          <tbody className="divide-y divide-gray-800">
            {group.standings.map((team, index) => (
              <tr
                key={team.teamId}
                className={`relative hover:bg-gray-800/50 cursor-pointer transition-all duration-300 ${
                  index < 2 ? 'bg-gray-800/20' : ''
                }`}
              >
                {/* Background image container */}
                ...
                <td className="py-4 text-left relative z-10 pl-2 align-middle">
                  <span
                    className={`font-valorant hover:text-primary transition-all duration-300 relative group ${
                      index < 2 ? 'text-white font-bold' : 'text-white'
                    }`}
                  >
                    {team.teamName}
                    <span className="absolute -bottom-1 font-base left-0 w-0 h-0.5 bg-primary group-hover:w-full transition-all duration-300"></span>
